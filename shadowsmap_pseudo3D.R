...
shadows=shadowsmap(DEM=DEM, dx=RESOLUTION, dlight=c(0, 40, 5))
...
writeTIFF(hillshade, "hillshadespike.tif", bits.per.sample=16, compression="LZW")


# 3. CALCULATE SHADOWS

shadows=shadowsmap(DEM=spike, dx=RESOLUTION, dlight=c(0, 30, 5))
writePNG(shadows, "shadowsEAST_spike_max.png")


# 4. CREATE SLICES

NSLIDES=400  # pixels
composite=readTIFF("composite_combined.tif")
R=composite[,,1]
G=composite[,,2]
B=composite[,,3]
...
stepmap[,,2]=Gd
stepmap[,,3]=Bd
writeTIFF(stepmap, "stepmap_combined.tif", bits.per.sample=16, compression="LZW")
